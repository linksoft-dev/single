...
func TestPagHiperCreate(t *testing.T) {
	...
	//billet, err = Paghiper.Create(billet)
...
func TestPagHiperStatus(t *testing.T) {
	...
	_, err := Paghiper.GetStatus(ValidPaghiperToken, InvalidPaghiperApikey, InvalidPaghiperTransactionId)
	assert.NotEqual(t, nil, err, "%sError: error is expected for non-existent transaction", testName)

	//finding with invalid token
	_, err = Paghiper.GetStatus(InvalidPaghiperToken, ValidPaghiperApikey, InvalidPaghiperTransactionId)
	assert.NotEqual(t, nil, err, "%sError: error is expected for non-existent transaction", testName)

	//finding inexistent transaction
	_, err = Paghiper.GetStatus(ValidPaghiperToken, ValidPaghiperApikey, InvalidPaghiperTransactionId)
	assert.NotEqual(t, nil, err, "%sError: error is expected for non-existent transaction", testName)

	//finding valid transaction
	responseMap, err := Paghiper.GetStatus(ValidPaghiperToken, ValidPaghiperApikey, ValidPaghiperTransactionId)
...
func TestPagHiperList(t *testing.T) {
	...
	resp, err := Paghiper.GetList(ValidPaghiperToken,
...
func TestPagHiperCancel(t *testing.T) {
	...
	_, err := Paghiper.Cancel(phToken, phApiKey, phTransactionId)
	...
	// isso está comentado pois nao deve se comunicar e fazer operacao com API externa dentro dos testes
	// mas deve ficar comentando pra ver como consumir o servico
	//respose, err := Paghiper.Cancel(phToken, phApiKey, phTransactionId)
}
